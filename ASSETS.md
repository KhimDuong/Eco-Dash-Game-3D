...
### C1 enemy kit — `Assets/Prefabs/Enemies/`
...
change instead of being re-dragged. C2/C3 extend the same builder.
...
Two things on the slime prefab are load-bearing and easy to delete by accident:

- **`NavMeshModifier(ignoreFromBuild)`** — `Level1Builder` bakes from physics
  colliders on every layer, so without it 29 slimes punch 29 holes in the NavMesh
  they walk on. `Player.prefab` carries one for the same reason.
